...
fig, axs = plt.subplots(1,numero_subplots, figsize=(width, height))
fig.subplots_adjust(left=.15, bottom=.16, right=.99, top=.97)

fig.suptitle(r'Lotka-Volterra model ($x_0 = {x0:.0f},\ y_0 = {y0:.0f}$)'\
...
# Graph limits
xmin = 0
xmax = max(prey)+max(prey)/100
ymin = 0
ymax = max(predator)+max(predator)/100

ax.plot(prey,predator,label='Foxes(rabbits)')
