module cholesky_mod
    ...
    contains
        subroutine cholesky(M, L, file_unit, output_format)
            ...
            integer :: i, j, k, n, totiter, rankM, cond1, superind
            !
            n = size(M,1)
            ...
            loopJ: do J=1, n
                ...
                !
                write(file_unit,'(1x,a,i0,a)') 'Matrix M^(',superind,')'
                lom2: do i = 1, n
                ...
                !
                if (cond1==0) then
                    write(file_unit,'(1x,a,i0,1x,a,i0)') &
                                                  'Vector L^',J,'≡ L^',superind
                else
                    write(file_unit,'(1x,a,i0)') 'Vector L^',superind
                end if
                !
                loopi: do i=1, n
                ...
                !
                write(file_unit,'(1x,a,i0,a)') 'New matrix M^(',superind,')'
                loopi2: do i = 1, n
                    loopk: do k = 1, n
                ...
                write(file_unit,*)
                !
                ! Rewrite L and M if M(J-1,J-1)<=0
                !
                if (cond1==0) then
                    !
                    L(:,superind) = L(:,J)
                    L(:,J) = 0.0_8
                    ! Move column
                    M(superind:n,superind+1:J) = M(superind:n,superind:J-1)
                    M(superind:n,superind) = 0.0_8
                    ! Move row
                    M(J:superind-1,superind+1:n) = M(J-1:superind,superind+1:n)
                    M(superind,superind+1:n) = 0.0_8
                    !
                    write(file_unit,'(1x,a,i0,a)') &
                                             'Rewrited matrix M^(',superind,')'
                    do i = 1, n
                        write(file_unit,actual_format) M(i,:)
                    end do
                end if
            ...
            write(file_unit,*)
            !
            write(file_unit,'(1x,a,i0)') 'Rank of M = ', rankM
            write(file_unit,*)
            !
